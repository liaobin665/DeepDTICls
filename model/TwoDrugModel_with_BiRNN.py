...
class MyTowDrugModel_with_BiRNN:
    def __init__(self, dropout=0.2, filters=128, decay=0.0, prot_len=2500, activation="relu", drug_len1=2048, drug_len2=300):
        ...
        self.__model_t = self.model(self.__dropout, self.__filters, prot_len=self.__prot_len,
                                    activation=self.__activation, drug_len1=self.__drug_len1, drug_len2=self.__drug_len2)
    ...
    def Conv(self, size, filters, activation, initializer, regularizer_param):
        ...
        return fun
    # 其中，drug1是需要进行encoding的，drug2是不用encoding， 是特征向量直接用于 分类的
    def model(self, dropout, filters, prot_len=2500, activation='relu', initializer="glorot_normal", drug_len1=2048, drug_len2=300):
        ...
        input_drug2 = Input(shape=(drug_len2,))
        ...
        model_d1 = Lambda(lambda x: K.expand_dims(x, axis=1))(input_drug)
        ...
        #第二个drug特征，加上一个双向循环网络提取特征
        layer_drug2 = Lambda(lambda x: K.expand_dims(x, axis=1))(input_drug2)
        layer_drug2 = Bidirectional(GRU(32))(layer_drug2)

        layer_drug = Concatenate(axis=1)([layer_drug, layer_drug2])
        ...
        layer_out = Model(inputs=[input_drug,input_drug2, input_protein], outputs=layer_compact)
        return layer_out

    def fit(self, drug_feature1, drug_feature2, protein_feature, label, test_drug1, test_drug2, test_protein, test_label,
            n_epoch=10, batch_size=32):
        for _ in range(n_epoch):
            history = self.__model_t.fit([drug_feature1, drug_feature2, protein_feature], label, epochs=_ + 1,
                                         batch_size=batch_size,
                                         validation_data=([test_drug1, test_drug2, test_protein], test_label), shuffle=True,
                                         verbose=1, initial_epoch=_)
        return self.__model_t

    def summary(self):
        self.__model_t.summary()

    def validation(self, drug_feature1,drug_feature2, protein_feature, label, test_drug1,test_drug2, test_pro, test_label, output_file=False, n_epoch=40, batch_size=128, **kwargs):
        epochs = []
        precisionScores = []
        recallScores = []
        accuracyScores = []
        ...
        for i_epoch in range(n_epoch):
            ...
            history = self.__model_t.fit([drug_feature1,drug_feature2, protein_feature], label,
                                         epochs=i_epoch + 1, batch_size=batch_size,
                                         validation_data=([test_drug1,test_drug2, test_pro], test_label),
            ...
            for dataset in kwargs:
                ...
                predictionResult = self.__model_t.predict([test_d1,test_d2, test_p])
